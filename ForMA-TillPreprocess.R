...
fcData$CoAge <- fct_collapse(fcData$CoAge,
                           ...
                           NotProvided = "8888",
                           NoCoApplicant = "9999")
levels(fcData$Age) <- c("25orLess", "65orMore","25to44","45to64","NotProvided")
levels(fcData$CoAge) <- c("25orLess", "65orMore","25to44","45to64","NotProvided","NoCoApplicant")
...
pData <- fcData %>%
  ...
  select(ID, everything()) #Moving ID to First Colummn

#Seperating State Column (to avoid encoding it)
enData <- pData %>% 
  select(!c(State, ActionTaken))
dm <- dummyVars("~.", data = enData)
trData <- data.frame(predict(dm, newdata = enData))

# ---- Attach State Column to trData
